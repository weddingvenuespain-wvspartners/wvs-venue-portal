...
const buildCss = (pri: string, priRgb: string, darkPri: boolean, font: string) => `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0 }
  ...
  .t5-nav.scrolled { border-bottom-color: ${LINE}; opacity: 1; pointer-events: auto }
  ...
  .t5-hero-img-overlay { position: absolute; inset: 0; pointer-events: none }
...
export default function T5Minimalista({ data }: { data: ProposalData }) {
  ...
  // Dynamic price: update when couple selects a date slot with its own price
    ...
    <div className="t5 tpl-root">
      ...
      <section className="t5-hero">
            ...
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '5px 14px', borderRadius: 999, background: `${primary}12`, border: `1px solid ${primary}22`, marginBottom: 22 }}>
              ...
              {data.venue?.name && (
                <>
                  ...
                  <span style={{ fontSize: '.68rem', fontWeight: 500, color: '#6b635a' }}>{data.venue.name}</span>
                </>
              )}
            </div>
        ...
        <div className="t5-hero-right">
          ...
          {(() => {
            const oColor = (sec as any).hero_overlay_color ?? primary
            const oAlpha = (sec as any).hero_overlay_opacity ?? 0.08
            const hex = oColor.replace('#','')
            const cr = parseInt(hex.slice(0,2),16), cg = parseInt(hex.slice(2,4),16), cb = parseInt(hex.slice(4,6),16)
            return <div className="t5-hero-img-overlay" style={{ background: `linear-gradient(to bottom right, rgba(${cr},${cg},${cb},${oAlpha.toFixed(2)}), transparent)` }} />
          })()}
        </div>
      </section>
            ...
            <div className="t5-pricing-inner">
              ...
              <div className="t5-receipt">
                ...
                <div className="t5-receipt-total">
                  ...
                  <span className="t5-receipt-total-val">{formatPrice(displayPrice ?? data.price_estimate)}</span>
                </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: 1, background: LINE, border: `1px solid ${LINE}`, maxWidth: 960, margin: '0 auto' }}>
              {collabsShow.map((c: any, i: number) => (
                <FadeUp key={i} delay={(i % 4) * .04}>
                  <div style={{ background: WHITE, padding: '22px 24px', height: '100%', ...(c.exclusive ? { borderLeft: `3px solid ${primary}` } : {}) }}>
                    {c.exclusive && <div style={{ fontSize: 9, fontWeight: 700, letterSpacing: '.15em', textTransform: 'uppercase', color: primary, marginBottom: 6 }}>★ Exclusivo</div>}
                    ...
                    {(c.phone || c.website || c.instagram || c.email) && (
                      <div style={{ display: 'flex', gap: 10, marginTop: 8, flexWrap: 'wrap' }}>
                        {c.phone && <a href={`tel:${c.phone}`} style={{ fontSize: 11, color: primary, textDecoration: 'none' }}>{c.phone}</a>}
                        {c.website && <a href={c.website.startsWith('http') ? c.website : `https://${c.website}`} target="_blank" rel="noopener noreferrer" style={{ fontSize: 11, color: primary, textDecoration: 'none' }}>Web ↗</a>}
                        {c.instagram && <a href={`https://instagram.com/${c.instagram.replace('@', '')}`} target="_blank" rel="noopener noreferrer" style={{ fontSize: 11, color: primary, textDecoration: 'none' }}>@{c.instagram.replace('@', '')}</a>}
                        {c.email && <a href={`mailto:${c.email}`} style={{ fontSize: 11, color: primary, textDecoration: 'none' }}>{c.email}</a>}
                      </div>
                    )}
                  </div>
                </FadeUp>
