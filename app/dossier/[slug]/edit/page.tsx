...
export default function ProposalEditarPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  useEffect(() => {
    ...
    if (!features.propuestas) { router.replace('/dossier'); return }
    const load = async () => {
  ...
  if (error) {
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 12, background: 'var(--cream)' }}>
        ...
        <button className="btn btn-ghost btn-sm" onClick={() => router.push('/dossier')}>← Volver a propuestas</button>
      </div>
    )
  }
