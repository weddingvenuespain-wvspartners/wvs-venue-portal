...
import { formatDate, formatPrice, isDark, toRgb, FadeUp, extractData, ConversionBlock, FloatingWhatsApp, AvailabilityBanner, type ProposalData } from './shared'
    ...
    <div style={{ fontFamily: 'Inter,system-ui,sans-serif', background: '#F8F6F3', minHeight: '100vh' }}>
      ...
      <section style={{ position: 'relative', height: '56vh', minHeight: 400, overflow: 'hidden', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
        ...
        <div style={{ position: 'absolute', top: 0, left: 0, right: 0, zIndex: 10, padding: '24px 48px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          {logo ? <img src={logo} alt="" style={{ height: 24, objectFit: 'contain', filter: 'brightness(0) invert(1)', opacity: .7 }} />
        ...
        <div style={{ position: 'relative', zIndex: 10, padding: '0 48px 44px', maxWidth: 900 }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 16, fontFamily: 'Inter,sans-serif', fontSize: 13, color: 'rgba(255,255,255,.5)' }}>
            {venue?.name && <span>📍 {venue.name}{venue.city?`, ${venue.city}`:''}</span>}
            {wDate && <span>📅 {wDate}</span>}
            {guest_count && <span>👥 {guest_count} invitados</span>}
          </div>
        </div>
      </section>
      ...
      <div className="main-layout">
        ...
        <main className="content">
          ...
          {on('gallery') && gallery.length > 0 && (
            <div style={{ display: 'grid', gridTemplateColumns: `repeat(${gallery.length},1fr)`, gap: 6, borderRadius: 14, overflow: 'hidden', marginBottom: 0, marginTop: !(on('experience') && expShow && (expShow as any).body) ? 0 : -24 }}>
              {gallery.map((url,i) => (
                <div key={i} style={{ height: 200, overflow: 'hidden' }}>
                  <img src={url} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block', transition: 'transform .6s', cursor: 'default' }}
              ...
              {!ctaSent ? (
                <FadeUp delay={.1}>
                  ...
                  <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                    {ctas.includes('visit') && <button className="btn btn-pri" onClick={()=>submitCta('visit')} disabled={sending||!ctaF.name||!ctaF.email}>{sending?'Enviando…':'📅 Solicitar visita'}</button>}
      ...
      <footer style={{ background: '#181410', padding: '40px 48px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 16 }}>
        <div>
          {logo && <img src={logo} alt="" style={{ height: 22, objectFit: 'contain', filter: 'brightness(0) invert(.3)', display: 'block', marginBottom: 8 }} />}
