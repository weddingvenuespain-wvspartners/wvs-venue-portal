  ...
  return (
    <div className="t1">
      ...
      <nav className={`t1-nav ${scrolled ? 'scrolled' : ''}`}>
        {logo
          ? <img src={logo} alt={venue?.name || ''} style={{ height: 28, objectFit: 'contain', filter: 'brightness(0) invert(.6)' }} />
      ...
      <section className="t1-hero">
        ...
        <div className="ha" style={{ position: 'absolute', top: 0, left: 0, right: 0, padding: '24px 48px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', zIndex: 10, animationDelay: '.1s' }}>
          {logo
            ? <img src={logo} alt="" style={{ height: 26, objectFit: 'contain', filter: 'brightness(0) invert(1)', opacity: .7 }} />
        ...
        <div style={{ position: 'relative', zIndex: 10, padding: '0 48px 80px' }}>
          ...
          <div className="ha" style={{ display: 'flex', flexWrap: 'wrap', gap: 16, alignItems: 'center', animationDelay: '.55s' }}>
            {venue?.city && <span style={{ fontSize: '.82rem', color: 'rgba(255,255,255,.4)', letterSpacing: '.04em' }}>📍 {venue.name}, {venue.city}</span>}
            {wDate && <span style={{ fontSize: '.82rem', color: 'rgba(255,255,255,.4)' }}>📅 {wDate}</span>}
            {guest_count && <span style={{ fontSize: '.82rem', color: 'rgba(255,255,255,.4)' }}>👥 {guest_count} invitados</span>}
            {show_price_estimate && price_estimate && (
          ...
          <div className="t1-story">
            ...
            {photos[1] && (
              <div className="t1-story-img">
                <img src={photos[1]} alt="El espacio" loading="lazy" />
      ...
      {/* ════════════════════════════════════════════
          GALERÍA MOSAICO
      ════════════════════════════════════════════ */}
      {on('gallery') && photos.length > 2 && (
        <FadeIn>
          <div className="t1-mosaic">
            {photos.slice(2, 7).map((url, i) => (
              <div key={i} className={`t1-mosaic-item${i === 0 ? ' span-2' : ''}`}>
                  ...
                  <div className="t1-zone">
                    <div className="t1-zone-img">
                      {zPhoto
                        ...
                        : <div className="t1-zone-ph">🏛️</div>
                      }
                    </div>
      ...
      <footer className="t1-footer">
        <div>
          {logo && <img src={logo} alt="" style={{ height: 20, objectFit: 'contain', filter: 'brightness(0) invert(.3)', marginBottom: 8, display: 'block' }} />}
