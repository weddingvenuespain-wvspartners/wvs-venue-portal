...
export default function T4SocialProof({ data }: { data: ProposalData }) {
  ...
  const galleryItems = photos.slice(0, 8).map((url, i) => ({
    url,
    ...
    <div className="t4">
        ...
        <div className="t4-hero-content">
          ...
          <FadeIn delay={0.3}>
            <div className="t4-hero-meta">
              {data.wedding_date && (
                <span className="t4-hero-pill">📅 {formatDate(data.wedding_date)}</span>
              )}
              {data.guest_count && (
                <span className="t4-hero-pill">👥 {data.guest_count} invitados</span>
              )}
      ...
      {photoCount > 1 && on('gallery') && (
        <section className="t4-gallery-bg">
          ...
          <div className="t4-gallery-strip">
            {galleryItems.slice(0, 4).map((item, i) => (
              <div key={i} className="t4-gallery-item">
                <img src={item.url} alt={item.coupleName || `Foto ${i + 1}`} loading="lazy" />
                {item.coupleName && (
                  <div className="t4-gallery-caption">
                    <div className="t4-gallery-caption-name">{item.coupleName}</div>
                    {item.weddingDate && <div className="t4-gallery-caption-date">{item.weddingDate}</div>}
                  </div>
                )}
              </div>
            ))}
                  ...
                  <div className={`t4-pkg ${pkg.is_recommended ? 'recommended' : ''}`}>
                    ...
                    {(pkg.min_guests || pkg.max_guests) && (
                      <div className="t4-pkg-guests">
                        👥 {pkg.min_guests && `${pkg.min_guests}`}{pkg.min_guests && pkg.max_guests && '–'}{pkg.max_guests && `${pkg.max_guests}`} invitados
                      </div>
                    )}
                  </div>
                </FadeUp>
                    ...
                    <div className="t4-zone">
                      <div className="t4-zone-img-wrap">
                        {zPhoto
                          ...
                          : <div className="t4-zone-ph">🏛️</div>
                        }
                      </div>
                      <div className="t4-zone-info">
                        ...
                        {(z.capacity_min || z.capacity_max) && (
                          <div className="t4-zone-cap">
                            👥 {z.capacity_min}{z.capacity_min && z.capacity_max ? '–' : ''}{z.capacity_max} personas
                          </div>
                        )}
