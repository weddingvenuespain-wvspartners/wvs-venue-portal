...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { date_from, date_to, price, notes, day_from, day_to, price_per_person, zone_prices, supplement_prices, group_prices, venue_id } = body
    ...
    const { data, error } = await svc
      ...
      .insert({
        ...
        group_prices:      group_prices      ?? null,
      })
      .select()
      .single()
